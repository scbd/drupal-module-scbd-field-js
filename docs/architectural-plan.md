...
A **deep module behind a DOM hydration contract**: a small stable surface hides all fetching,
shaping, localization, relation-linking, and write-back. The contract (selector convention,
...
in `src/utils/constants.js`. Per-site single-select therefore needs a Drupal-side code change —
recorded as a deferred item.

Hidden behind the seam: which vocabularies are fetched and how, term localization and shaping,
grouped nesting, GBF auto-linking, legacy SDG migration, and every failure path. The host only ever
sees a mounted app reading and writing one hidden input.
...
Both actors reach the widget through Drupal; the site manager configures domain order and the two
auto-add toggles at `/admin/config/scbd-field`.
...
rendered and configured multi-select; add-only and one-way.
...
A **domain** is one vocabulary; its **terms** are trimmed to `{ identifier, name }` (grouped
domains carry `children`). The **hidden input** stores identifiers joined by commas — national
...
`api.cbd.int` seam is read-only HTTPS. The widget writes nothing back except the selection into the
local hidden input.
...
Only `change-case`, `ofetch`, and `vue-multiselect` are bundled; Vue is external.
