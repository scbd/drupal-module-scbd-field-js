...
> **⚠ Target-state PRD (v3.0.0) — this branch is v1.0.0.** This document specifies the **target
> v3.0.0** widget; the code on this branch is the **v1.0.0** bundle (one grouped
> `@scbd/cached-apis` multiselect). The in-module data layer, `singleValueDomains`, `GBF_SAMEAS`,
...
ecosystem types, and others. A plain Drupal text or taxonomy field cannot do this: the vocabularies
live in an external API, term names are translated into dozens of languages, a GBF target maps to
...
**Hidden-input contract.** The widget reads and writes its selection through the hidden Drupal
input located by field name — deliberately no initial-value or auto-add props. The host page owns
preloading (`#default_value`) and new-entity seeding; the input is the single source of truth and
`name` the only required prop.
...
PurgeCSS. A release publishes only the minified script, the stylesheet, and a checksum; the source
map is built `hidden` — on disk for debugging, excluded from the artifact, release assets, and npm
...
- One bundle, two site shapes: BL2 (Belgium, four locales) and BSL (six UN languages, grouped
  biosafety subjects). The dev harness encodes both plus an "unused domains" view so every domain
  is reachable while testing.
