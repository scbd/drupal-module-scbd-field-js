...
> _This doc is **not** a pull request and not a code review — it is the plan for *how to make* the PRs._
...
> **Heads-up on the base branch.** Branch `latest` already holds a **46-commit** version of this same
...
> trunk-based development: the shared branch must remain shippable after every merge.
...
**Six facts shape every option; the first four are hard constraints, not preferences:**
...
- Keep the **widget-importing tests and runtime-dep changes OUT of B1** (fact #4) — otherwise B1's
  `vitest run` fails against the old widget.
