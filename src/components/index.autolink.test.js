...
// Mounts the App wrapper (@/index.vue), not the child component directly. The wrapper forwards
// singleValueDomains to the child <ChmComponent> via an explicit prop binding (added in task p01-02),
// so the singleValueDomains passed in these tests reaches the autoLinkRelated guard under test.
...
async function selectGbfTarget() {
  const gbf = wrapper.findAllComponents(Multiselect)[0];
