...
export const relatedKeys = (identifier) => {
  // Guarded own-property read (SEC-3 / CWE-1321): a bare GBF_SAMEAS[id] returns Object.prototype /
  // the constructor / a function for id === '__proto__' / 'constructor' / 'toString', and the
  // caller's `new Set(relatedKeys(...))` would throw on a non-iterable. Resolve own array values only.
  if (!Object.prototype.hasOwnProperty.call(GBF_SAMEAS, identifier)) return [];
  const related = GBF_SAMEAS[identifier];
