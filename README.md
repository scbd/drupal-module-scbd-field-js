...
| `description`        | String  | `''`          | Help text rendered above the widget (empty by default — no help block renders). |
...
```js
// value2 widget — identical mount plus isAdditionalField (and singleValueDomains if overriding):
createApp(App, {
  name:              s.element_title,
  description:       s.element_description,
  countries:         s.countries ?? ['be'],
  ...
  locales:           s.locales   ?? ['en'],
  domains:           s.domains   ?? ['gbfTargets', 'nationalTargets7', 'countries', 'subjects', 'sdgs'],
  isAdditionalField: true,                  // read/write the value2 column
  // singleValueDomains: s.singleValueDomains, // optional per-site override
  debug:             s.debug     ?? false,
}).mount(`#scbd-field-thesaurus-${s.element_title}`);
```
...
vue:
  ...
  version: "3.5.25"
  js:
    https://unpkg.com/vue@^3.5.25/dist/vue.global.prod.js: { type: external, minified: true }
  css:
    component:
      https://unpkg.com/vue-multiselect@3.5.0/dist/vue-multiselect.css: { type: external, minified: true }
